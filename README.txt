...
padIn (bytes) (Decryption only): Encrypted padded bytes. Used in CBC and ECB modes in cases where the image does not have the proper size. Not always required, but the pad bytes will not be automatically generated if needed. 
...
padOut (bytes): Encrypted pad values. Required for EBC and CBC encryption and decryption.
